...
void main() {
  ...
  group('Barline fallback', () {
    test('8 stemless noteheads with no barlines split into 2 measures of 4 (default 4/4)', () {
      ...
      expect(part.measures.length, 2, reason: 'default 4/4 → 8 notes split into 2×4');
      expect(part.measures[0].notes.length, 4);
      expect(part.measures[1].notes.length, 4);
    });

    test('timeSigCutCommon (2/2) causes 6 notes to split into 3 measures of 2', () {
      final detection = DetectionResult(
        ...
        symbols: [
          sym(id: 'clef', type: 'gClef', x: 5, y: 92, width: 20, height: 48),
          sym(id: 'ts', type: 'timeSigCutCommon', x: 28, y: 108, width: 12, height: 24),
          for (int i = 0; i < 6; i++)
    ...
    test('signature symbols (clef) are placed in the first measure by the fallback', () {
      final detection = DetectionResult(
        ...
        symbols: [
          sym(id: 'clef', type: 'gClef', x: 5, y: 92, width: 20, height: 48),
          for (int i = 0; i < 4; i++)
            sym(id: 'n$i', type: 'noteheadWhole', x: 50.0 + i * 12, y: 126, width: 12, height: 8),
        ],
      );
  ...
  group('Signature inheritance', () {
    ...
    setUp(() {
      threeBarDetection = DetectionResult(
        ...
        symbols: [
          ...
          sym(id: 'clef', type: 'gClef', x: 5, y: 92, width: 20, height: 48),
          sym(id: 'ts-top', type: 'timeSig4', x: 30, y: 106, width: 10, height: 12),
          sym(id: 'ts-bot', type: 'timeSig4', x: 30, y: 120, width: 10, height: 12),
          sym(id: 'n1', type: 'noteheadWhole', x: 55, y: 126, width: 12, height: 8),
          // Measure 2 — no signature symbols
          sym(id: 'n2', type: 'noteheadWhole', x: 100, y: 126, width: 12, height: 8),
          // Measure 3 — no signature symbols
          sym(id: 'n3', type: 'noteheadWhole', x: 145, y: 126, width: 12, height: 8),
        ],
      );
    });
    ...
    test('key signature is carried forward to measures without one', () {
      final detection = DetectionResult(
        ...
        symbols: [
          sym(id: 'clef', type: 'gClef', x: 5, y: 92, width: 20, height: 48),
          sym(id: 'ks', type: 'accidentalSharp', x: 28, y: 112, width: 8, height: 16),
          sym(id: 'n1', type: 'noteheadWhole', x: 55, y: 126, width: 12, height: 8),
          sym(id: 'n2', type: 'noteheadWhole', x: 100, y: 126, width: 12, height: 8),
          sym(id: 'n3', type: 'noteheadWhole', x: 145, y: 126, width: 12, height: 8),
        ],
  ...
  group('Context-aware duration for stemless noteheadBlack', () {
    ...
    test('stemless noteheadBlack as last symbol with 2 beats remaining → half', () {
    ...
    test('stemless noteheadBlack as last with only 1 beat remaining → quarter (fallback)', () {
      ...
      expect(notes[3].type, 'quarter', reason: 'only 1 beat remaining → quarter fallback');
    });
  });
}
